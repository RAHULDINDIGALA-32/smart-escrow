...
contract Escrow is ReentrancyGuard {
    ...
    error Escrow__OracleNotResolved();
    ...
    constructor(
        ...
        oracleId = keccak256(abi.encode(address(this)));
        state = State.CREATED;
    }
    ...
    function oracleProposeRelease() external {
        if (state != State.DISPUTED) {
            revert Escrow__InvalidState();
        }

        oracle.propose(oracleId, OptimisticOracle.Outcome.RELEASE);
    }

    function oracleProposeRefund() external {
        if (state != State.DISPUTED) {
            revert Escrow__InvalidState();
        }
        oracle.propose(oracleId, OptimisticOracle.Outcome.REFUND);
    }

    function finalize() external nonReentrant {
        OptimisticOracle.Outcome outcome = oracle.finalize(oracleId);
        _execute(outcome);
    }

    function resolveViaResolver(OptimisticOracle.Outcome outcome) external nonReentrant {
        OptimisticOracle.Outcome finalOutcome = oracle.resolveDispute(oracleId, outcome);
        _execute(finalOutcome);
    }
    ...
    function _execute(OptimisticOracle.Outcome outcome) internal {
        if (state != State.DISPUTED) {
            revert Escrow__InvalidState();
        }

        // if (!oracle.proposals(oracleId).resolved) {
        //     revert Escrow__OracleNotResolved();
        // }

        state = State.RESOLVED;

        if (outcome == OptimisticOracle.Outcome.RELEASE) {
            _payout(i_beneficiary);
        } else {
            _payout(i_depositor);
        }
